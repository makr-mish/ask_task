...
export async function POST(req: NextRequest) {
  try {
    ...
    const url = `${API_BASE}/yandex_browser/getfreefeedback/${userIdText}/${token}`;
    ...
    const resultValue =
      data?.result ?? data?.data?.result ?? data?.id_yandex_browser ?? data?.data?.id_yandex_browser;
      ...
      return NextResponse.json(
        {
          ok: false,
          error:
            data?.message || data?.error || `Ошибка внешнего API: ${externalRes.status}`,
          data,
        },
    ...
    return NextResponse.json({
      ...
      data: {
        ...
        id_yandex_browser: String(resultValue),
      },
      raw: data,
    });
  } catch (error) {
    console.error("yandex-browser get-feedback error:", error);
    return NextResponse.json(
      {
        ok: false,
