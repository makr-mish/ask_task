...
export async function POST(req: NextRequest) {
  try {
    ...
    try {
      ...
      return NextResponse.json({ ok: false, error: "Некорректный ответ сервера" }, { status: 500 });
    }

    if (!res.ok) {
      return NextResponse.json(
        { ok: false, error: data?.error || data?.message || "Ошибка отправки на модерацию" },
        { status: res.status },
      );
    }
    ...
    console.error("yandex-browser update-review-status error:", error);
    return NextResponse.json({ ok: false, error: "Ошибка отправки на модерацию" }, { status: 500 });
  }
}
