...
export async function POST(req: NextRequest) {
  try {
    ...
    const res = await fetch(
      `https://lk.rating-smart.ru:8080/api/bot/gettask/${userIdText}/${token}/dream_job`,
      {
        method: "POST",
        headers: {
    ...
    console.error("dream-job get-task error:", error);

    return NextResponse.json(
      { ok: false, error: "Ошибка получения задания" },
      { status: 500 },
    );
  }
}